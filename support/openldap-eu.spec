...
%define ldapdatadir      /var/lib/ldap
...
%define check_password_conf         /etc/openldap/check_password.conf
...
./configure --enable-ldap --enable-debug --sysconfdir=/etc --prefix=%{ldapserverdir} --libdir=%{ldapserverdir}/%{_lib} --with-tls --with-cyrus-sasl --enable-spasswd --enable-overlays --enable-modules --enable-slapi --enable-meta --enable-crypt
make depend
make %{?_smp_mflags}
# check_password
...
# Init script changes
sed -i 's:^SLAPD_CONF=.*:SLAPD_CONF="/etc/openldap/slapd.conf"': %{buildroot}/etc/init.d/slapd
#sed -i 's:^SLAPD_PATH.*:SLAPD_PATH="'%{ldapdir}'":' %{buildroot}/etc/init.d/slapd
# Init config changes
...
sed -i 's:^SLAPD_CONF=.*:SLAPD_CONF="/etc/openldap/slapd.conf"': %{buildroot}/etc/default/slapd
sed -i 's-^SLAPD_SERVICES.*-SLAPD_SERVICES="ldap://$IP:$PORT ldaps://$SSLIP:$SSLPORT ldapi:///"-' %{buildroot}/etc/default/slapd
...
sed -i 's:^directory.*:directory\t'%{ldapdatadir}':' %{buildroot}/etc/openldap/slapd.conf
...
%exclude %{ldapdir}/src
%exclude %{ldapdir}/lib/debug
# Excluded for RH openldap package conflicts
%exclude /usr/share/man/man5/ldap.conf.5.gz
%exclude /usr/share/man/man5/ldif.5.gz
%exclude /etc/openldap/ldap.conf

%{ldapdir}
%config(noreplace) /etc/openldap/slapd.conf
...
#=================================================
%changelog
* Tue Jun 11 2013 - Justin Lambert <jlambert@letsevenup.com> - 2.4.35-1
- Renamed to -eu
- Changed install locations of files
- Removed rsyslog config
